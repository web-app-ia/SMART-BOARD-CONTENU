@echo off
title Smart Board - Détection de Poses
color 0A

:: ================================================
:: SMART BOARD UNIVERSAL LAUNCHER TEMPLATE
:: Application: Détection de Poses
...
set "APP_PATH=%BASE_PATH%\SMART_BOARD_Apps\body_tracking\pose_detection"
...
echo.
echo ================================================================
echo    SMART BOARD - Détection de Poses
echo ================================================================
echo.
...
echo [INFO] Démarrage de Détection de Poses...
...
"%PYTHON_ENV%\Scripts\python.exe" "main.py" 2>"%LOGS_PATH%\pose_detection_error.log"

:: Gestion des codes de sortie
if %errorLevel% neq 0 (
    echo.
    echo [ERREUR] L'application s'est terminée avec une erreur (Code: %errorLevel%)
    echo [INFO] Vérifiez le fichier de log: %LOGS_PATH%\pose_detection_error.log
    pause
    exit /b %errorLevel%
)
