@echo off
title Smart Board - Contrôle Gestuel
color 0A

:: ================================================
:: SMART BOARD UNIVERSAL LAUNCHER TEMPLATE
:: Application: Contrôle Gestuel
...
set "APP_PATH=%BASE_PATH%\SMART_BOARD_Apps\body_tracking\gesture_control"
...
echo.
echo ================================================================
echo    SMART BOARD - Contrôle Gestuel
echo ================================================================
echo.
...
echo [INFO] Démarrage de Contrôle Gestuel...
...
"%PYTHON_ENV%\Scripts\python.exe" "main.py" 2>"%LOGS_PATH%\gesture_control_error.log"

:: Gestion des codes de sortie
if %errorLevel% neq 0 (
    echo.
    echo [ERREUR] L'application s'est terminée avec une erreur (Code: %errorLevel%)
    echo [INFO] Vérifiez le fichier de log: %LOGS_PATH%\gesture_control_error.log
    pause
    exit /b %errorLevel%
)
