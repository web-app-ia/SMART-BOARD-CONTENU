@echo off
title Smart Board - Overlay AR
color 0A

:: ================================================
:: SMART BOARD UNIVERSAL LAUNCHER TEMPLATE
:: Application: Overlay AR
:: Category: ar_vr
:: Version: 1.0.0
:: ================================================
...
set "APP_PATH=%BASE_PATH%\SMART_BOARD_Apps\ar_vr\ar_overlay"
...
echo [INFO] Démarrage de Overlay AR...
...
"%PYTHON_ENV%\Scripts\python.exe" "main.py" 2>"%LOGS_PATH%\ar_overlay_error.log"

:: Gestion des codes de sortie
if %errorLevel% neq 0 (
    echo.
    echo [ERREUR] L'application s'est terminée avec une erreur (Code: %errorLevel%)
    echo [INFO] Vérifiez le fichier de log: %LOGS_PATH%\ar_overlay_error.log
    pause
    exit /b %errorLevel%
)
