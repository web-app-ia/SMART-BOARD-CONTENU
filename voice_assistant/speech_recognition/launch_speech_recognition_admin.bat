@echo off
title Smart Board - Reconnaissance Vocale
color 0A
...
set "APP_PATH=%BASE_PATH%\SMART_BOARD_Apps\voice_assistant\speech_recognition"
...
echo.
echo ================================================================
echo    SMART BOARD - Reconnaissance Vocale
echo ================================================================
echo.
...
echo [INFO] Démarrage de Reconnaissance Vocale...
...
"%PYTHON_ENV%\Scripts\python.exe" "main.py" 2>"%LOGS_PATH%\speech_recognition_error.log"

:: Gestion des codes de sortie
if %errorLevel% neq 0 (
    echo.
    echo [ERREUR] L'application s'est terminée avec une erreur (Code: %errorLevel%)
    echo [INFO] Vérifiez le fichier de log: %LOGS_PATH%\speech_recognition_error.log
    pause
    exit /b %errorLevel%
)
